...
def combine_pymutations(pymut1: PyMutation, pymut2: PyMutation) -> PyMutation:
    ...
    """
    logger.info(f"Starting combination of PyMutation instances: {pymut1.metadata.file_path} and {pymut2.metadata.file_path}")
    if pymut1.metadata.assembly != pymut2.metadata.assembly:
        logger.error(f"Cannot combine PyMutation instances with different assemblies: {pymut1.metadata.assembly} and {pymut2.metadata.assembly}")
        raise ValueError(
            f"Cannot combine PyMutation instances with different assemblies: "
    ...
    logger.info(f"Combined samples: {len(pymut1.samples)} from first instance + {len(pymut2.samples)} from second instance = {len(combined_samples)} unique samples")
    ...
    df1['_variant_id'] = df1.apply(
        lambda row: f"{row['CHROM']}_{row['POS']}_{row['REF']}_{row['ALT']}", 
        axis=1
    )
    df2['_variant_id'] = df2.apply(
        lambda row: f"{row['CHROM']}_{row['POS']}_{row['REF']}_{row['ALT']}", 
        axis=1
    )
    logger.info(f"Created unique variant identifiers for {len(df1)} variants in first instance and {len(df2)} variants in second instance")
    ...
    logger.info(f"Column analysis: {len(common_cols)} common columns, {len(unique_cols1)} unique to first instance, {len(unique_cols2)} unique to second instance")
    ...
    logger.info(f"Processed {len(common_cols)} common columns, {len(unique_cols1)} columns unique to first instance, and {len(unique_cols2)} columns unique to second instance")
    ...
    annotation_columns = [col for col in combined_df.columns 
                         if col not in std_columns and col not in combined_samples]
    ...
    logger.info(f"Reordered columns: {len(std_columns)} standard columns + {len(combined_samples)} sample columns + {len(annotation_columns)} annotation columns")
    
    combined_metadata = MutationMetadata(
        source_format=f"COMBINED {pymut1.metadata.source_format} + {pymut2.metadata.source_format}",
        file_path=f"{pymut1.metadata.file_path} + {pymut2.metadata.file_path}",
        filters=["."],
    ...
    logger.info(f"Combination complete: created PyMutation instance with {len(combined_df)} variants and {len(combined_df.columns)} columns")
